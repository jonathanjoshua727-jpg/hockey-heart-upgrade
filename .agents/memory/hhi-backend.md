...
- Public donor settings may expose method availability, public wallet addresses, and explicitly confirmed provider labels only—never bank/account/routing/SWIFT/instruction data or cached admin details.
- Bank transfer stays disabled until the live provider supports USD. An admin-edited provider label is display-only and must match the secure server integration; changing it never swaps gateways or credentials.
