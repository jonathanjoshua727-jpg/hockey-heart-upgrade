...
- contentStore keeps legacy `paystackPublicKey`/`paystackEnabled` fields for localStorage compat but nothing reads them anymore; method gating uses only the enable toggles (keys live server-side).
