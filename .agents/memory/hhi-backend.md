...
- Confirmation email: atomic claim on `email_sent_at IS NULL` before sending; sent via the Replit-managed Resend connection (`@replit/connectors-sdk` proxy, POST /emails). Send verified working. Sender defaults to `onboarding@resend.dev` (override with `DONATION_EMAIL_FROM` once a custom domain is verified in Resend); reply-to is hockeyheartinitiative@gmail.com. Note: managed Resend key rejects /domains (401) — only sending is allowed; test sends with `delivered@resend.dev`.
