...
export function CampaignCard({
  ...
  const href = slug ? `/campaigns/${slug}` : `/campaigns/${id}`;

  const isHockey =
    category.toLowerCase().includes("hockey") ||
    category.toLowerCase().includes("equipment") ||
    category.toLowerCase().includes("ice") ||
  ...
  return (
    <div className="group flex flex-col bg-card text-card-foreground rounded-2xl border border-card-border overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
      ...
      <div className="flex flex-col flex-1 p-6">
        <Link href={href}>
          <h3 className="font-serif text-xl font-bold mb-2 text-primary hover:text-primary/80 transition-colors">
            {title}
          </h3>
        </Link>
        <p className="text-muted-foreground text-sm line-clamp-3 mb-6 flex-1">{description}</p>
        ...
        <div className="space-y-3 mb-6">
          ...
          <div className="text-xs text-muted-foreground text-right">Deadline: {deadline}</div>
        </div>
