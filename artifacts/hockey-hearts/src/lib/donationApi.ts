...
function pct(num: number, total: number): number {
  ...
  return Math.min(100, Math.max(0, Math.round((num / total) * 100)));
}
  ...
  return {
    ...
    counts: {
      successful: safeNumber(
        (rawCounts as Partial<DonationStats["counts"]>).successful,
      ),
      pending: safeNumber(
        (rawCounts as Partial<DonationStats["counts"]>).pending,
      ),
      failed: safeNumber(
        (rawCounts as Partial<DonationStats["counts"]>).failed,
      ),
      cancelled: safeNumber(
        (rawCounts as Partial<DonationStats["counts"]>).cancelled,
      ),
      refunded: safeNumber(
        (rawCounts as Partial<DonationStats["counts"]>).refunded,
      ),
    },
